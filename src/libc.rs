...
extern "C" {
    ...
    fn hash_function(input: *const c_char, output: *mut u8);
...
pub fn get_hash_hex(input: &str) -> String {
    let mut output = [0u8; 32]; // 256 bits for SHA256
    unsafe {
        hash_function(input.as_ptr() as *const libc::c_char, output.as_mut_ptr());
